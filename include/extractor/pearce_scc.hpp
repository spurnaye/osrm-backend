...
template <typename GraphT> class PearceSCC
{
  ...
  public:
    PearceSCC(std::shared_ptr<const GraphT> graph)
    ...
    void Run()
    {
        ...
        std::stack<std::tuple<NodeID, typename GraphT::EdgeRange, bool>> recursion_stack;
        ...
        for (const auto u : util::irange(0u, max_node_id))
        {
            if (rindex[u] == 0)
            {
                ...
                while (!recursion_stack.empty())
                {
                    ...
                    const auto v = std::get<0>(frame);
                    for (auto edge : std::get<1>(frame))
                    {
                        ...
                        {
                            rindex[v] = rindex[w];
                            std::get<2>(frame) = false; // root = false
                        }
                    }
